        ...
        <div className="flex flex-col gap-4">
          <div className="w-full overflow-x-auto hide-scrollbar">
            <div className="min-w-[850px]">
              {/* Table Header */}
              <div className="grid grid-cols-[20px_2fr_2fr_1fr_1.5fr_1.5fr_140px] text-left gap-4 px-4 py-2 text-xs font-semibold text-color-secondary border-b border-gray-700">
                <span></span>
                <span>Full Name</span>
                <span>Email</span>
                <span>Phone</span>
              ...
              {isLoading ? (
                ...
                <div className="divide-y divide-gray-800">
                  {paginatedProfiles.map((profile) => (
                    <div
                      key={profile.id}
                      className="grid grid-cols-[20px_2fr_2fr_1fr_1.5fr_1.5fr_140px] gap-4 px-4 py-3 text-left text-sm hover:bg-gray-800 transition-all cursor-pointer group"
                      onClick={() => navigate(`/${currentWorkspace.url}/profiles/${profile.id}`)}
                    >
                      <div><BoxIcon size={18} className="text-gray-500" /></div>
                      ...
                      <div className="text-color-secondary">{profile.department}</div>
                      <div className="text-color-secondary">{profile.yearLevel} - {profile.section}</div>
                      <div className="flex items-center justify-center gap-2 pr-4">
                        <ActionsMenu selectedProfile={profile} />
                      </div>
                    </div>
                  ))}
                </div>
