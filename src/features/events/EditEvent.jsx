...
function EditEvent({ open, onClose, event }) {
  ...
  const handleUpdate = async () => {
    ...
    // Date validation (allow original date even if in past)
    if (!eventDate) {
      ...
      // Only validate if date changed
      if (eventDate !== originalEventDate && inputDate < today) {
          ...
          >
            ...
            <div className="space-y-2 sm:space-y-3 flex-1">
              <div className='p-1 bg-gray-700 rounded w-fit'>
                <BoxIcon size={20} className='text-gray-500'/>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <ReactDatePicker
                  ...
                  onChange={(date) => {
                    ...
                    // Only validate if date changed
                    else if (selectedDate !== originalEventDate && inputDate < today) {
                      setEventDateError("Event date cannot be in the past");
                    } else {
                      setEventDateError("");
                    }
